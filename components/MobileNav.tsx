...
// Import specific type for Icons
...
interface MobileNavProps {
  menuItems: MenuItem[]
  role: 'student' | 'admin'
  userEmail?: string // Optional: Pass email for a better footer look
}
...
export function MobileNav({ menuItems, role, userEmail }: MobileNavProps) {
  ...
  // 1. SCROLL LOCK: Prevent body scroll when menu is open
  useEffect(() => {
    if (isOpen) {
            ...
            >
                   ...
                   return (
                     <motion.div key={item.href} variants={itemVariants}>
                       <Link 
                         href={item.href}
                         onClick={() => setIsOpen(false)}
                         className={`group flex items-center justify-between px-4 py-3.5 rounded-xl font-medium transition-all duration-200 ${
                           isActive 
                       ...
                       >
                         ...
                         {/* Small arrow for non-active items */}
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50/50">
                 <div className="flex items-center gap-3 mb-5 p-3 bg-white rounded-xl border border-gray-200 shadow-sm">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 to-gray-300 flex items-center justify-center text-gray-500">
                        <UserCircle size={24} />
                    </div>
                 ...
                 <button 
                   ...
                   className="w-full flex items-center justify-center gap-2 bg-white border border-red-100 py-3.5 rounded-xl text-red-600 font-semibold text-sm hover:bg-red-50 hover:border-red-200 transition-all active:scale-95"
                 >
